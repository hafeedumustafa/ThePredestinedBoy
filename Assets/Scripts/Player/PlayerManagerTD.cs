...
public class PlayerManagerTD : MonoBehaviour
{

    public SpriteRenderer Face;
    public Sprite UpFace;
    public Sprite DownFace;
    public Sprite HorizontalHead;
    ...
    void Update()
    {
        ...
        if(InputByKeyboard) {
            if(CanMove) {
                ...
                if(VerticalMovement < 0) {LastKeyPressed = -2; Face.sprite = DownFace;} 
                else if(VerticalMovement > 0) {LastKeyPressed = 2; Face.sprite = UpFace;} 

                if(HorizontalMovement < 0) {
                    transform.localScale = new Vector3(-0.2f, 0.2f, 1);
                    LastKeyPressed = 1;
                    Face.sprite = HorizontalHead;
